...
class LSTM(nn.Module):
    ...
    def forward(self, x):
        ...
        if (lstm_output.shape[2] == 2 * self.n_hidden):  # This way it works for both bidirectional and a unidirectional LSTM
            batch_size = lstm_output.shape[0]
            lstm_output = lstm_output.view(batch_size, 2, self.n_hidden)  # batch size, num_directions, n_hidden)
            lstm_output = lstm_output[:, 0, :].view(batch_size, 1, self.n_hidden)
